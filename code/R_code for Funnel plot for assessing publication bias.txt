# ============================================
# Egger's test for publication bias (PFT transformation)
# Generates PDF and TIFF funnel plots
# ============================================
...
for (sheet_name in sheet_names) {
  ...
  tryCatch({
    ...
    if (nrow(df) >= 3) {
      # PFT transformation (Freeman-Tukey double arcsine)
      ...
      # TIFF funnel plot (high resolution)
...
if (nrow(results) > 0) {
  ...
  cat("- Publication bias detected (p < 0.05):", sum(results$Bias == "Yes"), "\n")
  cat("- No publication bias (p ≥ 0.05):", sum(results$Bias == "No"), "\n")
  cat("- Mean studies per sheet:", round(mean(results$Studies), 1), "\n")
  cat("- Mean I²:", round(mean(results$I2, na.rm = TRUE), 1), "%\n")
  cat("- Mean τ²:", round(mean(results$Tau2, na.rm = TRUE), 4), "\n")
  ...
  # Plot 3: Study count distribution
  ...
  # Plot 4: P-value vs heterogeneity
